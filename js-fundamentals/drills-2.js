...
// ]
//
function createListOfObjects(arrayOfStrings) {
...
var createSpanTags = function(articleTags) {
    var output = ''

    articleTags.forEach(function(article) {
        output += '<span class="article-tags">' + article + '</span>'
    })

    return output

    // return articleTags.reduce(function(previous, next) {
    //     return '<span class="article-tags">' + previous + '</span>' + (i === 0 ? '<span class+"article-tags">' + next + '</span>' : '')
    // })
}
